...
type WizardStep = 'loading' | 'create-agent' | 'business-details' | 'test-call' | 'calendar' | 'checklist';
...
export default function SetupWizard() {
  ...
  function determineStep(s: ProvisionStatus) {
    if (!s.provisioned) {
      setStep('create-agent');
    } else if (!s.checklist.businessDetailsAdded) {
      setStep('business-details');
    } else if (!s.checklist.testCallMade) {
      setStep('test-call');
    } else if (!s.isLive) {
      setStep('checklist');
    } else {
  ...
  // If live, show minimal status instead of wizard
  if (status?.isLive) {
    return (
    ...
    <div className="bg-white rounded-2xl border-2 border-[#0d9488] overflow-hidden">
      ...
      <div className="bg-[#1a2e3b] px-6 py-5">
        <h2 className="text-[18px] font-bold text-white mb-1">
          ...
          {step === 'business-details' && 'Tell us about your business'}
          {step === 'test-call' && `Test ${displayName}`}
          {step === 'checklist' && 'Ready to go live'}
        </h2>
        <p className="text-[13px] text-white/60">
          {step === 'checklist' 
            ? 'Review your setup and activate when ready'
            : `Step ${currentStepNum} of ${totalSteps}`
          }
        </p>
        <div className="mt-3 h-1.5 bg-white/10 rounded-full overflow-hidden">
          <div className="h-full bg-[#0d9488] rounded-full transition-all duration-500" style={{ width: `${(currentStepNum / totalSteps) * 100}%` }} />
        </div>
      </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <input
                type="text"
                value={agentName}
                onChange={e => setAgentName(e.target.value.slice(0, 20))}
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {nameSuggestions.slice(0, 6).map(n => (
                  <button key={n} onClick={() => setAgentName(n)} className={`text-[12px] px-3 py-1 rounded-full border transition-colors ${agentName === n ? 'bg-[#0d9488] text-white border-[#0d9488]' : 'bg-[#f8f7f6] text-[#5a7184] border-[#e5e0da] hover:border-[#0d9488]'}`}>{n}</button>
                ))}
              </div>
            </div>
        ...
        {step === 'business-details' && (
          <div className="space-y-5">
            ...
            <div className="bg-[#f0fdf4] rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-[13px] font-semibold text-[#059669]">{displayName} is created!</p>
                <p className="text-[13px] text-[#5a7184]">Number: <span className="font-bold text-[#1a2e3b]">{displayPhone}</span></p>
              </div>
            </div>

            <p className="text-[14px] text-[#5a7184] leading-relaxed">
              {displayName} already knows your trade — now add your specific business details so {displayName.toLowerCase() === displayName ? displayName : 'she'} sounds like part of your team.
            </p>

            {/* Owner phone */}
            <div>
              <label className="block text-sm font-semibold text-[#1a2e3b] mb-1">Your mobile number <span className="text-red-500">*</span></label>
              <p className="text-[12px] text-[#94a7b8] mb-1.5">We&apos;ll text you when calls come in</p>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input type="range" min="5" max="75" value={serviceRadius} onChange={e => setServiceRadius(e.target.value)}
            ...
            <div>
              ...
              <textarea value={customFAQ} onChange={e => setCustomFAQ(e.target.value)}
                placeholder="e.g. Licensed &amp; insured, 10+ years experience, same-day service available, senior discounts..."
                rows={2}
                className="w-full px-4 py-2.5 border border-[#e5e0da] rounded-xl text-[14px] text-[#1a2e3b] focus:outline-none focus:ring-2 focus:ring-[#0d9488] resize-none" />
            </div>

            <button onClick={handleSaveDetails} disabled={saving || !ownerPhone.trim()}
          ...
          <div className="space-y-5">
            ...
            <div className="text-center py-4">
              <p className="text-[14px] text-[#5a7184] mb-4">
                Give {displayName} a test call — pretend to be a customer and see how {displayName.toLowerCase() === displayName ? displayName : 'she'} handles it.
              </p>
              ...
              <p className="text-[12px] text-[#94a7b8] mt-4">
                Tip: Try asking about pricing, booking an appointment, or asking if you serve their area.
              </p>
            </div>
        ...
        {step === 'checklist' && status && (
          <div className="space-y-5">
            ...
            {!status.isLive && (
              <div className="bg-[#fef3c7] border border-[#fbbf24] rounded-xl p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-[#a16207]">Only you can call {displayPhone}. Real customers won&apos;t reach {displayName} until you go live.</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <ChecklistItem done={status.checklist.ownerPhoneSet} label="Your phone number added" sublabel="SMS alerts for incoming calls" onFix={() => setStep('business-details')} />
              <ChecklistItem done={status.checklist.businessDetailsAdded} label="Business details added" sublabel="Pricing, services, service area" onFix={() => setStep('business-details')} />
            ...
            {/* Go live button */}
            <div className="pt-2">
              <button onClick={handleGoLive} disabled={goingLive || !status.checklist.businessDetailsAdded || !status.checklist.ownerPhoneSet}
              ...
              <p className="text-[12px] text-[#94a7b8] text-center mt-2">
                After going live, set up <a href="/dashboard/setup" className="text-[#0d9488] font-medium no-underline hover:underline">call forwarding</a> so {displayName} answers your main number.
              </p>
            </div>
